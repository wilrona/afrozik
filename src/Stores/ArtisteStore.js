...
class ArtisteStore {
	...
	@observable offset = 0;
	@observable limit = 5;
	@observable by = 'nom_artiste_public' // 'nom_artiste_public', 'pays', 'date_creation'
	...
	@action getById = () => {
		...
		Api.get('/artiste/detail/'+this.id, Config).then(({data}) => {
			runInAction(() => {
...
let artisteStore = new ArtisteStore();
